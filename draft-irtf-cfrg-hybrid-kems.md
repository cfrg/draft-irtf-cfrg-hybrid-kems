...
- `New(seed) -> state`: Produce a new PRG using `seed` as the initial entropy.
- `Read(state, n) -> out`: A deterministic algorithm to generate an `n`-byte
  string of pseudo-random bytes.  The `state` value may be mutated by this
  function.
...
Given a string `seed` of (pseudo)randomly-generated bytes, one can define a
trivial PRG that simply reads from those bytes:
...
- `GenerateKeyPair(prg) -> (ek, dk)`: A deterministic algorithm that takes as
  input a mutable PRG state `prg` and generates a public encapsulation key `ek`
  and a secret decapsulation key `dk`, each of which are byte strings.
- `Encaps(ek, prg) -> (ct, ss)`: A probabilistic encapsulation algorithm, which
  takes as input a public encapsulation key `ek` and a pseudorandom generator
  `prg` and outputs a ciphertext `ct` and shared secret `ss`.
...
The `GenerateKeyPair` and `Encaps` functions can be derandomized by supplying
them with an instance of the trivial PRG described in {{prgs}}.  When this
derandomization is done, the input MUST have a minimum length specified by the
definition of the KEM:

- `Nseed`: The minimum length in bytes of a seed for a `GenerateKeyPair` operation
- `Nrandom`: The minimum length in bytes of a seed for an `Encaps` operation
...
- `RandomScalar(prg) -> k`: A deterministic algorithm to produce a uniform
  pseudo-random scalar from the pseudo-random generator `prg`.
...
The `RandomScalar` function can be derandomized by supplying it with an instance
of the trivial PRG described in {{prgs}}.  When this derandomization is done,
the seed provided to the trivial PRG MUST have a minimum length specified by the
definition of the group:

- `Nseed`: The minimum length in bytes of a seed for a `RandomScalar` operation
...
GenerateKeyPair, Encap, and Decap functions of PQ KEMs, alongside analogous
...
GenerateKeyPair, Encap, and Decap functions of the traditional and PQ KEMs in
parallel.
...
def expandDecapsKeyK(seed):
    prg = TrivialPRG.new(seed)
    (ek_PQ, dk_PQ) = KEM_PQ.GenerateKeyPair(prg)
    (ek_T, dk_T) = KEM_T.GenerateKeyPair(prg)
    return (ek_PQ, ek_T, dk_PQ, dk_T)
...
The GenerateKeyPair methods in the frameworkss below describe "shared seed" key
generation, where the decapsulation key for they hybrid KEM is a seed from which
subkeys for the PQ and T components are generated via a PRG.  In some deployment
environments, it is not possible to instantiate this process, e.g., because the
external entropy cannot be supplied to the component algorithms.
...
def Encaps(ek):
    ...
    # TODO(RLB) Wrong arguments to universal combiner
    ss_H = UniversalCombiner(ss_PQ, ss_T, ct_T, ek_T, Label))
    ct_H = concat(ct_PQ, ct_T)
    return (ss_H, ct_H)

def Decaps(dk, ct):
    # TODO(RLB) Missing split of ciphertext
